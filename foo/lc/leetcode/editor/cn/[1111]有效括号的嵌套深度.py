# A string is a valid parentheses string (denoted VPS) if and only if it consist
# s of "(" and ")" characters only, and: 
# 
#  
...
class Solution:
    def maxDepthAfterSplit(self, seq: str) -> List[int]:
        result = [0] * len(seq)
        # 要求 两个分开的数组 所以根据奇偶对(进行区分
        for i in range(len(seq)):
            result[i] = i & 1 if seq[i] == '(' else (i + 1) & 1
        return result

a = Solution().maxDepthAfterSplit("(((()))((())))")
print(a)
